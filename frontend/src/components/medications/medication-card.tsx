      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <Badge className={medication.status === "completed" ? "bg-muted text-muted-foreground" : "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"}>{medication.status}</Badge>
          </div>
          ...
          {isLow && <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">Only {medication.remainingDoses} doses remaining — plan your refill.</p>}
        </div>
