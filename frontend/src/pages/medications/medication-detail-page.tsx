...
const statusStyle: Record<AdherenceStatus, string> = { taken: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300", skipped: "bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-300", snoozed: "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300" };
...
export function MedicationDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {lowDoses && <section className="flex flex-col justify-between gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:flex-row sm:items-center dark:border-amber-900 dark:bg-amber-950/30"><div className="flex gap-3"><PackagePlus className="mt-1 shrink-0 text-amber-700" /><div><h2 className="font-bold text-amber-950 dark:text-amber-100">Time to plan a refill</h2><p className="mt-1 text-sm text-amber-800 dark:text-amber-300">You have {medication.remainingDoses} doses left. Send a refill request before you run out.</p></div></div><Button>Request refill</Button></section>}
